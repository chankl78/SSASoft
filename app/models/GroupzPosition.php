...
class GroupzPosition extends Eloquent {

	protected $table = 'Group_z_Position';
        ...
        static::updating(function($record)
        {
            try
            {
                ...
                foreach ($dirty as $field => $newdata)
                {
                    ...
                    if ($olddata != $newdata)
                    {
                        LogsfLogs::postLogs('Update', 43, $record->id, ' - Group Position - From:  ' . $field . ' - From ' . $olddata . ' To: ' . $newdata, $olddata, $newdata, 'Success');
                    }
                }
                return true;
            }
            ...
            {
                LogsfLogs::postLogs('Update', 43, $record->id, ' - Group Position - ' . $field . ' - ' . $e, $olddata, $newdata, 'Failed');
            }
        });
    }
