...
class EventMemRegistrationController extends BaseController {
	...
	public function postChingayAuthorizationCheck()
	{
		try
		{
			$eventid = ConfigurationmDefault::ChingayDefaultCode();
		...
		{
			LogsfLogs::postLogs('Create', 28, 0, ' - Event Chingay Registration - Authorization Check Failed - ' . $e, NULL, NULL, 'Failed');
			return Response::json(array('info' => 'Failed', 'ErrType' => 'Unknown'), 400);
		}
	}
	...
	public function getChingayIndex()
	{
		$view = View::make('eventregistration/eventchingayregistration');
		$view->title = 'Chingay 2022 Registration';
		$rhq_options = MemberszOrgChart::Rhq()->lists('rhq', 'rhqabbv');
		$zone_options = MemberszOrgChart::Zone()->lists('zone', 'zoneabbv');
		$chapter_options = MemberszOrgChart::Chapter()->lists('chapter', 'chapabbv');
		$memposition_options = MemberszPosition::orderBy('code', 'ASC')->lists('name', 'code');
	...
	public function postChingayMember()
	{
		try
		{
			if(EventmRegistration::getCheckSignature(EventmRegistration::getid(Input::get('memberid'))) == false)
			{
				...
				$post->parentsignature = Input::get('parentsignature');
				$post->parentrelationship = Input::get('parentrelationship');
				$post->parentsignaturesigned = date('Y-m-d H:i:s');
